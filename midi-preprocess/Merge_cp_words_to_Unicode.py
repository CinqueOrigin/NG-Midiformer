...
def convert_CpWords_to_Unicode(words_dir,output_dir,dict_file=None,baseNum=1000):
    ...
    if dict_file is None:
        ...
        with open(os.path.join(output_dir,'mergeUnicodeFilePiano.txt'),'w') as fout:
            for file in tqdm(word_files):
                Words=[]
                with open(os.path.join(words_dir,file),'r') as f:
                    for line in f:
                        line = line.replace('\n','')
                        # print(line)
                        word=line.split(' ')
        ...
        with open(os.path.join(output_dir,'label.txt'),'w') as f_label:
            word2unic,unic2word = pickle.load(open(dict_file,'rb'))
            index=baseNum
            #3.allocate unicode to a cpWord
            cnt=0
            os.makedirs(output_dir, exist_ok=True)
            print(os.path.join(output_dir,'mergeUnicodeFilePiano.txt'))
            with open(os.path.join(output_dir,'mergeUnicodeFilePiano.txt'),'w') as fout:
                for file in tqdm(word_files):
                    Words=[]
                    with open(os.path.join(words_dir,file),'r') as f:
