"""PyTorch NG-Midiformer model classes."""
...
class NG_MidiformerEncoder(nn.Module):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        super(NG_MidiformerEncoder, self).__init__()
        self.output_attentions = output_attentions
...
class NG_MidiformerOnlyMLMHead(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        super(NG_MidiformerOnlyMLMHead, self).__init__()
...
class NG_MidiformerOnlyNSPHead(nn.Module):
    def __init__(self, config):
        super(NG_MidiformerOnlyNSPHead, self).__init__()
...
class NG_MidiformerPreTrainingHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        super(NG_MidiformerPreTrainingHeads, self).__init__()
        self.predictions = BertLMPredictionHead(config, bert_model_embedding_weights)
        self.seq_relationship = nn.Linear(config.hidden_size, 2)
...
class NG_MidiformerPreTrainedModel(PreTrainedModel):
    config_class = NG_MidiformerConfig
...
class NG_MidiformerModel(NG_MidiformerPreTrainedModel):
    """
    Inputs:
    ...
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        super(NG_MidiformerModel, self).__init__(config)
        self.output_attentions = output_attentions
        self.embeddings = BertEmbeddings(config)
        self.word_embeddings = BertWordEmbeddings(config)
        self.encoder = NG_MidiformerEncoder(config, output_attentions=output_attentions,
                                  keep_multihead_output=keep_multihead_output)
    ...
    def get_input_embeddings2(self):
        return self.word_embeddings.word_embeddings

    def set_input_embeddings2(self, value):
        self.word_embeddings.word_embeddings = value
    ...
    #         output_embeddings2 = self.get_output_embeddings2()
    ...
    def resize_token_embeddings(self, new_num_tokens: Optional[int] = None,new_num_ngrams: Optional[int] = None) -> nn.Embedding:
        """
        Resizes input token embeddings matrix of the model if `new_num_tokens != config.vocab_size`.

        Takes care of tying weights embeddings afterwards if the model class has a `tie_weights()` method.
        """
        model_embeds,model_embeds2 = self._resize_token_embeddings(new_num_tokens,new_num_ngrams)
        if new_num_tokens is None:
            return model_embeds

        self.config.vocab_size = new_num_tokens
        self.vocab_size = new_num_tokens
        self.config.word_size = new_num_ngrams
        self.word_size = new_num_ngrams

        self.tie_weights()

    def _resize_token_embeddings(self, new_num_tokens,new_num_ngrams):
        old_embeddings = self.get_input_embeddings()
        new_embeddings = self._get_resized_embeddings(old_embeddings, new_num_tokens)
        self.set_input_embeddings(new_embeddings)

        old_embeddings2 = self.get_input_embeddings2()
        new_embeddings2 = self._get_resized_embeddings(old_embeddings2, new_num_ngrams)
        self.set_input_embeddings2(new_embeddings2)

        return self.get_input_embeddings(),self.get_input_embeddings2()

class NG_MidiformerForPreTraining(NG_MidiformerPreTrainedModel):
    """
    Inputs:
        `input_ids`: a torch.LongTensor of shape [batch_size, sequence_length]
            with the word token indices in the vocabulary
    ...
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        super(NG_MidiformerForPreTraining, self).__init__(config)
        self.output_attentions = output_attentions
        self.bert = NG_MidiformerModel(config, output_attentions=output_attentions,
                             keep_multihead_output=keep_multihead_output)
        self.cls = NG_MidiformerPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight)
        self.init_weights()
...
class NG_MidiformerForMaskedLM(NG_MidiformerPreTrainedModel):
    """
    Inputs:
        `input_ids`: a torch.LongTensor of shape [batch_size, sequence_length]
            with the word token indices in the vocabulary
    ...
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        super(NG_MidiformerForMaskedLM, self).__init__(config)
        self.output_attentions = output_attentions
        self.bert = NG_MidiformerModel(config, output_attentions=output_attentions,
                             keep_multihead_output=keep_multihead_output)
        self.cls = NG_MidiformerOnlyMLMHead(config, self.bert.embeddings.word_embeddings.weight)
        self.init_weights()

    def forward(self, input_ids, input_ngram_ids, ngram_position_matrix, token_type_ids=None, attention_mask=None, ngram_attention_mask=None, 
    ...
    def get_input_embeddings2(self):
        return self.bert.word_embeddings.word_embeddings

    def set_input_embeddings2(self, value):
        self.bert.word_embeddings.word_embeddings = value
    ...
    def resize_token_embeddings(self, new_num_tokens: Optional[int] = None,new_num_ngrams: Optional[int] = None) -> nn.Embedding:
        ...
        self.bert.resize_token_embeddings(new_num_tokens,new_num_ngrams)
        self.tie_weights()
...
def convert_example_for_input(examples,max_seq_length,tokenizer,ngram_dict):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens = ["[CLS]"] + tokenizer.convert_ids_to_tokens(example)[0:max_seq_length-2] + ["[SEP]"]
...
def convert_example_to_input(examples,max_seq_length=512,tokenizer=None,ngram_dict=None):
    ...
    ngram_positions_matrix = torch.zeros(
        size=(batch_size,max_seq_length,ngram_dict.max_ngram_in_seq),
        dtype=torch.int)
    ...
    return  ngram_ids,ngram_positions_matrix,input_ids


class NG_MidiformerForMaskedLMSmall(NG_MidiformerPreTrainedModel):
    """    
    Inputs:
        `input_ids`: a torch.LongTensor of shape [batch_size, sequence_length]
            with the word token indices in the vocabulary
    ...
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        super(NG_MidiformerForMaskedLMSmall, self).__init__(config)
        self.output_attentions = output_attentions
        self.bert = NG_MidiformerModel(config, output_attentions=output_attentions,
                             keep_multihead_output=keep_multihead_output)
        self.cls = NG_MidiformerOnlyMLMHead(config, self.bert.embeddings.word_embeddings.weight)
    ...
    def forward(self, input_ids,token_type_ids=None, attention_mask=None, ngram_attention_mask=None,
    labels=None, head_mask=None):
        input_ngram_ids,ngram_position_matrix,input_ids_new = convert_example_to_input(examples=input_ids,max_seq_length=512,tokenizer=self.tokenizer,ngram_dict=self.ngramDict)
        input_ngram_ids = input_ngram_ids.to(input_ids.device)
        ngram_position_matrix = ngram_position_matrix.to(input_ids.device)
        input_ids_new = input_ids_new.to(input_ids.device)
        labels = torch.cat((torch.IntTensor([[-100]]*input_ids.size(0)).to(input_ids.device),labels,torch.IntTensor([[-100]]*input_ids.size(0)).to(input_ids.device)),dim=1)
        outputs = self.bert(input_ids_new, input_ngram_ids, ngram_position_matrix, None, None, None, ngram_attention_mask,
                            output_all_encoded_layers=False,
                            head_mask=head_mask)
        if self.output_attentions:
            all_attentions, sequence_output, _ = outputs
...
def buildNgramPositions(batch_size,max_seq_length,max_ngram_in_seq,ngram_ids,ngram_starts,
    ngram_lengths,ngram_freqs,device):
    ngram_positions_matrix = torch.zeros(
        size=(batch_size,max_seq_length,max_ngram_in_seq),
        dtype=torch.int)
    ngram_positions_matrix = ngram_positions_matrix.to(device)
    for batch_id in range(batch_size):
        ngram_id = ngram_ids[batch_id]
        ngram_start = ngram_starts[batch_id]
        ngram_length = ngram_lengths[batch_id]
    ...
    return ngram_positions_matrix

class NG_MidiformerForMaskedLMModified(NG_MidiformerPreTrainedModel):
    """   
     Inputs:
        `input_ids`: a torch.LongTensor of shape [batch_size, sequence_length]
            with the word token indices in the vocabulary
    ...
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        super(NG_MidiformerForMaskedLMModified, self).__init__(config)
        self.output_attentions = output_attentions
        self.bert = NG_MidiformerModel(config, output_attentions=output_attentions,
                             keep_multihead_output=keep_multihead_output)
        self.cls = NG_MidiformerOnlyMLMHead(config, self.bert.embeddings.word_embeddings.weight)
        self.max_ngram_in_seq =128
        self.max_seq_length = 1024
        self.init_weights()
    def forward(self, input_ids,ngram_ids,ngram_starts,ngram_freqs,ngram_lengths,token_type_ids=None,
     attention_mask=None, ngram_attention_mask=None,
     labels=None, batch_mask=None,head_mask=None):
        # print('runing')
        # print(self.bert.encoder.layer[0].attention.self.query.weight.dtype)
        ngram_position_matrix = buildNgramPositions(len(input_ids),self.max_seq_length,self.max_ngram_in_seq,
            ngram_ids,ngram_starts,ngram_lengths,ngram_freqs,input_ids.device)
        # labels = torch.cat((torch.IntTensor([[-100]]*input_ids.size(0)).to(input_ids.device),labels,torch.IntTensor([[-100]]*input_ids.size(0)).to(input_ids.device)),dim=1)
        outputs = self.bert(input_ids, ngram_ids, ngram_position_matrix, None, None, None, ngram_attention_mask,
                            output_all_encoded_layers=False,
                            head_mask=head_mask)
        if self.output_attentions:
            all_attentions, sequence_output, _ = outputs
        ...
        if labels is not None:
            if batch_mask is None:
                loss_fct = CrossEntropyLoss()
                masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), labels.view(-1))
                # print(masked_lm_loss.dtype)
                return MaskedLMOutput(loss=masked_lm_loss,logits=prediction_scores)
            else:
                loss_fct = CrossEntropyLoss(reduction="none")
                masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), labels.view(-1))
                masked_lm_loss = masked_lm_loss.view(input_ids.size(0),input_ids.size(1))
                masked_lm_loss *=batch_mask
                seq_loss= masked_lm_loss.sum(dim=1)/batch_mask.sum(dim=1)
                seq_loss = seq_loss.mean()
                # print(masked_lm_loss.dtype)
                return MaskedLMOutput(loss=seq_loss,logits=prediction_scores)
        elif self.output_attentions:
            return all_attentions, prediction_scores
        output = (prediction_scores,) + outputs[2:]
        return ((masked_lm_loss,) + output) if masked_lm_loss is not None else output
...
class NG_MidiformerForNextSentencePrediction(NG_MidiformerPreTrainedModel):
    """
    Inputs:
        `input_ids`: a torch.LongTensor of shape [batch_size, sequence_length]
            with the word token indices in the vocabulary
    ...
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        super(NG_MidiformerForNextSentencePrediction, self).__init__(config)
        self.output_attentions = output_attentions
        self.bert = NG_MidiformerModel(config, output_attentions=output_attentions,
                             keep_multihead_output=keep_multihead_output)
        self.cls = NG_MidiformerOnlyNSPHead(config)
        self.init_weights()
...
class NG_MidiformerForNextSentencePredictionModified(NG_MidiformerPreTrainedModel):
    """ 
        Inputs:
        `input_ids`: a torch.LongTensor of shape [batch_size, sequence_length]
            with the word token indices in the vocabulary
    ...
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        super(NG_MidiformerForNextSentencePredictionModified, self).__init__(config)
        self.output_attentions = output_attentions
        self.bert = NG_MidiformerModel(config, output_attentions=output_attentions,
                             keep_multihead_output=keep_multihead_output)
        self.cls = NG_MidiformerOnlyNSPHead(config)
...
class NG_MidiformerForSequenceClassification(NG_MidiformerPreTrainedModel):
    """
    Inputs:
        `input_ids`: a torch.LongTensor of shape [batch_size, sequence_length]
    ...
    def __init__(self, config, num_labels=2, output_attentions=False, keep_multihead_output=False):
        super(NG_MidiformerForSequenceClassification, self).__init__(config)
        self.output_attentions = output_attentions
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets,reduction='sum'):
...
class NG_MidiformerForSequenceClassificationWithMultiLabel(NG_MidiformerPreTrainedModel):
    """
    Inputs:
        `input_ids`: a torch.LongTensor of shape [batch_size, sequence_length]
            with the word token indices in the vocabulary. Items in the batch should begin with the special "CLS" token. (see the tokens preprocessing logic in the scripts
    ...
    def __init__(self, config, num_labels=2, output_attentions=False, keep_multihead_output=False):
        super(NG_MidiformerForSequenceClassificationWithMultiLabel, self).__init__(config)
        self.output_attentions = output_attentions
        self.num_labels = config.num_labels
        self.bert = NG_MidiformerModel(config, output_attentions=output_attentions,
                             keep_multihead_output=keep_multihead_output)
    ...
    def forward(self, input_ids, input_ngram_ids, ngram_position_matrix, token_type_ids=None,\
                 attention_mask=None, labels=None, head_mask=None,weight_mask=None):
        outputs = self.bert(input_ids, input_ngram_ids, ngram_position_matrix, token_type_ids,
                            attention_mask=attention_mask,
                            output_all_encoded_layers=False,
                            head_mask=head_mask)
        ...
        logits = self.classifier(pooled_output)
        loss = None
        # print(self.bert.encoder.layer[8].attention.self.position_embedding.weights.shape)
        if labels is not None:
            if weight_mask is not None:
                loss_fct = BCEWithLogitsLoss(weight=weight_mask,reduction='sum')
                loss = loss_fct(logits.view(-1, self.num_labels), labels.float())
            else:
            # loss_fct = nn.functional.binary_cross_entropy_with_logits
                loss_fct = BCEWithLogitsLoss(reduction='sum')
                loss = loss_fct(logits.view(-1, self.num_labels), labels.float())
                # print(logits.view(-1, self.num_labels)[0])
            # print(labels.float()[0])
            return loss, logits
        elif self.output_attentions:
            return all_attentions, logits
        return loss, logits
...
class NG_MidiformerForTokenClassification(NG_MidiformerPreTrainedModel):
    """
    Inputs:
        `input_ids`: a torch.LongTensor of shape [batch_size, sequence_length]
            with the word token indices in the vocabulary
    ...
    def __init__(self, config, num_labels=2, output_attentions=False, keep_multihead_output=False):
        super(NG_MidiformerForTokenClassification, self).__init__(config)
        self.output_attentions = output_attentions
        self.num_labels = config.num_labels
        self.bert = NG_MidiformerModel(config, output_attentions=output_attentions,
                             keep_multihead_output=keep_multihead_output)
...
class NG_MidiformerForQuestionAnswering(NG_MidiformerPreTrainedModel):
    """
    Inputs:
        `input_ids`: a torch.LongTensor of shape [batch_size, sequence_length]
    ...
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        super(NG_MidiformerForQuestionAnswering, self).__init__(config)
        self.output_attentions = output_attentions
        self.bert = NG_MidiformerModel(config, output_attentions=output_attentions,
                             keep_multihead_output=keep_multihead_output)
