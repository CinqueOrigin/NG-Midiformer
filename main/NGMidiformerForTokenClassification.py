...
from model.model import NG_MidiformerForTokenClassification
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, ngram_ids, ngram_positions, ngram_lengths=None,
                 ngram_tuples=None, ngram_seg_ids=None, ngram_masks=None, valid_ids=None, label_mask=None, b_use_valid_filter=False):
...
def convert_examples_to_features(examples, label_map, max_seq_length, tokenizer, ngram_dict):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for i, word in enumerate(textlist.split(' ')):
            token = tokenizer.tokenize(word)
            if len(ntokens) + len(token) > max_seq_length - 1:
                break
            ntokens.extend(token)
            if i == len(labellist):
                print('exiting with',i,'and',labellist)
            label_1 = labellist[i]
        ...
        max_ngram_in_seq_proportion = math.ceil((len(ntokens) / max_seq_length) * ngram_dict.max_ngram_in_seq)
...
class DataProcessor(object):
    ...
    def get_examples(self, data_path, set_type, quotechar=' '):
        ...
        return self._create_examples(
            self._read_json(data_path), set_type)
...
class VelocityProcessor(DataProcessor):
    """Processor for the OntoNotes4 data set."""

    def get_labels(self):
        return [0,1,2,3,4,5,6,'[CLS]', '[SEP]'] 
...
class MelodyProcessor(DataProcessor):
    """Processor for the OntoNotes4 data set."""

    def get_labels(self):
        return [0,1,2,3,'[CLS]', '[SEP]'] 
...
class TaskDataModel(pl.LightningDataModule):
    ...
    def __init__(self, args):
        ...
        processors = {
            ...
            'Velocity': VelocityProcessor,
            'Melody':MelodyProcessor,
...
class LitModel(pl.LightningModule):
    ...
    def __init__(self, args, id2label):
        super().__init__()
        self.model = NG_MidiformerForTokenClassification.from_pretrained(args.pretrained_model_path, num_labels=len(id2label))
    ...
    def comput_metrix(self, y_pred, y_true):
    ...
    #     acc = self.comput_metrix(torch.masked_select(preds,batch['attention_mask'].byte().detach().cpu()),
    #                              torch.masked_select(labels,batch['attention_mask'].byte().detach().cpu()))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Masked selection for predictions and labels based on attention mask
        attention_mask = batch['attention_mask'].bool().detach().cpu()
        masked_preds = torch.masked_select(preds, attention_mask)
        masked_labels = torch.masked_select(labels, attention_mask)

        # Calculate accuracy
        acc = self.comput_metrix(masked_preds, masked_labels)

        # Flatten the outputs for precision and recall calculations
        flat_preds = masked_preds.view(-1)
        flat_labels = masked_labels.view(-1)

        # We only consider the positive label, which is '1'
        # You might need to adjust this depending on how your labels are defined
        positive_label = 1

        # Calculate TP, FP, and FN for precision and recall
        TP = (flat_preds == positive_label) & (flat_labels == positive_label)
        FP = (flat_preds == positive_label) & (flat_labels != positive_label)
        FN = (flat_preds != positive_label) & (flat_labels == positive_label)

        return {
            'val_loss': loss,
            'val_acc': acc,
            'TP': TP.sum().item(),  # Convert from tensor to Python scalar
            'FP': FP.sum().item(),
            'FN': FN.sum().item()
        }
...
def main():
    ...
    if args.ckpt_path is None:
        model = LitModel(args,id2label = id2label)
    else:
